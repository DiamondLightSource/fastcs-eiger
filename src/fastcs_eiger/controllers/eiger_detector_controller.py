...
def command_uri(api_version: str, key: str) -> str:
...
class EigerDetectorController(EigerSubsystemController):
    ...
    async def initialize(self):
        await self.connection.put(command_uri(self.api_version, key="initialize"))

    @detector_command
    async def arm(self):
        await self.connection.put(command_uri(self.api_version, key="arm"))
        ...
        match self.trigger_mode.get(), self.trigger_exposure.get():
            case ("inte", exposure) if exposure > 0.0:
                await self.connection.put(
                    command_uri(self.api_version, key="trigger"), exposure
                )
            case ("ints" | "inte", _):
                await self.connection.put(command_uri(self.api_version, key="trigger"))
            case _:
                raise RuntimeError("Can only do soft trigger in 'ints' or 'inte' mode")

    @detector_command
    async def disarm(self):
        await self.connection.put(command_uri(self.api_version, key="disarm"))

    @detector_command
    async def abort(self):
        await self.connection.put(command_uri(self.api_version, key="abort"))

    @detector_command
    async def cancel(self):
        await self.connection.put(command_uri(self.api_version, key="cancel"))
