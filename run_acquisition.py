...
async def run_acquisition(
    ...
    await pv_equals(f"{eiger_prefix}:StaleParameters", False)
    ...
    await start_writing(odin_prefix)
...
async def tidy(eiger_prefix: str, odin_prefix: str):
    await stop_writing(odin_prefix)
    await caput(f"{eiger_prefix}:Detector:Abort", True)


async def start_writing(pv_prefix: str):
    await asyncio.gather(
        caput(f"{pv_prefix}:FP:0:HDF:StartWriting", True),
        caput(f"{pv_prefix}:FP:1:HDF:StartWriting", True),
        caput(f"{pv_prefix}:FP:2:HDF:StartWriting", True),
        caput(f"{pv_prefix}:FP:3:HDF:StartWriting", True),
    )


async def stop_writing(pv_prefix: str):
    await asyncio.gather(
        caput(f"{pv_prefix}:FP:0:HDF:StopWriting", True),
        caput(f"{pv_prefix}:FP:1:HDF:StopWriting", True),
        caput(f"{pv_prefix}:FP:2:HDF:StopWriting", True),
        caput(f"{pv_prefix}:FP:3:HDF:StopWriting", True),
    )
